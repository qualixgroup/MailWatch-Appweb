  ...
  return (
    <div className="flex flex-col gap-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {stats.map((s, i) => (
          <div key={i} className="bg-surface-dark p-4 rounded-xl border border-border-dark shadow-lg">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs font-medium text-text-dim">{s.label}</p>
      ...
      <div className="bg-surface-dark border border-border-dark rounded-xl shadow-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm">
            <thead className="bg-background-dark/50 border-b border-border-dark text-text-dim uppercase text-[10px] font-bold tracking-widest">
              <tr>
            ...
            <tbody className="divide-y divide-border-dark">
              {logs.map((log) => (
                <tr key={log.id} className="hover:bg-white/5 transition-colors">
                  <td className="px-6 py-4 whitespace-nowrap text-text-dim font-mono text-xs">
                    {log.timestamp}
                  </td>
                  ...
                  <td className="px-6 py-4 text-white font-medium">
                    {log.title}
                    <p className="text-xs text-text-dim font-normal mt-0.5">{log.description}</p>
                  </td>

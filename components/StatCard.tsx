...
const StatCard: React.FC<StatCardProps> = ({ label, value, icon, color, sub, change }) => {
    return (
        <div className="bg-white dark:bg-surface-dark border border-gray-200 dark:border-border-dark rounded-xl p-6 relative overflow-hidden group hover:border-primary/50 transition-all duration-300">
            <div className={`absolute top-0 right-0 p-6 opacity-10 group-hover:opacity-20 transition-opacity ${color}`}>
                <span className="material-symbols-outlined text-6xl">{icon}</span>
            </div>
            <div className="flex flex-col gap-1 relative z-10">
                <p className="text-gray-500 dark:text-text-dim text-sm font-medium">{label}</p>
                ...
                <p className="text-gray-500 dark:text-text-dim text-xs mt-2">{sub || 'Dados em tempo real'}</p>
            </div>
        </div>
    );
};
